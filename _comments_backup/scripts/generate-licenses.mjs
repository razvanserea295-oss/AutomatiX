#!/usr/bin/env node
/**
 * Generates src/assets/licenses.json from package.json + Cargo.lock.
 *
 * Runs as part of prebuild:electron so the About page always has a fresh
 * attribution list. The output is a small array of
 * { name, version, license, author, homepage } objects — we don't ship the
 * full license text of every package (would be several MB) but we do ship
 * the critical ones (fonts, model weights, top-level deps) verbatim in
 * src/assets/licenses-full/.
 */
...
/**
 * Cargo.lock has one [[package]] block per dependency. We don't need the
 * license field for a proper SBOM (Cargo.lock doesn't carry it — we'd need
 * cargo-license). Instead we list names + versions so About page shows
 * what's in the Rust binary; license lookup can be done by URL if needed.
 */
