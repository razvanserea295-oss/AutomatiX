 ...
 * Legacy page-shell grammar (pre-migration) and the single full-width 1/2/3
        ...
        <div className={`${PAGE_GRID_12} items-stretch`}>
          <section className="xl:col-span-8 min-w-0 min-h-0 flex flex-col">
            <Panel
              ...
              title="Catalog template-uri"
              subtitle={`${filtered.length} ${filtered.length === 1 ? 'template' : 'template-uri'}${search ? ` găsite pentru „${search}"` : ''}`}
              bodyClassName="p-4"
            >
                {loading ? (
                    ...
                    {filtered.map(t => {
                      ...
                      return (
                        <button
                          ...
                          type="button"
                          onClick={() => select(t.id)}
                          style={{ viewTransitionName: isSel ? vtName('fisa-template', t.id) : undefined }}
                          className={`group ${LIST_TILE} flex flex-col p-4 text-left w-full transition-smooth duration-150 active:scale-[0.99] focus-visible:outline-none focus-visible:shadow-[var(--ring-soft)] ${isSel ? 'ring-1 ring-accent/40 bg-accent/5 vt-morph' : ''}`}
                        >
          ...
          <aside className="xl:col-span-4 min-h-0 flex flex-col">
            {!selected ? (
              <Panel fill className="flex-1 min-h-0" title="Selectează un template" subtitle="din catalog pentru detalii rapide">
              ...
              <Panel
                ...
                bodyClassName="px-4 py-3 flex-1 min-h-0 overflow-y-auto"
              >
                  {selected.is_default && (
                ...
                <div className="mt-4 pt-3 flex items-center gap-2 shrink-0 border-t border-line/60">
...
function PromptDialogHost() {
  ...
  return (
    <div
      className="fixed inset-0 z-[1200] flex items-center justify-center bg-black/55 p-4 anim-fade-in"
      onClick={() => close(null)}
      role="presentation"
    >
      <div
