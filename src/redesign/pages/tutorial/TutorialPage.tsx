/**
 * TutorialPage — Modern-SaaS rebuild (RE-ARRANGED layout).
 *
 * Read-only help / onboarding hub. Fully static: NO apiCommand, NO fetch, NO
 * stores. All content lives in three in-file constant arrays (LIFECYCLE_SECTIONS,
 * PAGES_GUIDE, TIPS) and the unified `buildSearchIndex` helper — every one of
 * them is preserved VERBATIM below.
 *
 * ─────────────────────────────────────────────────────────────────────────────
 * The user's #1 complaint was "the card POSITIONS haven't changed". The original
 * was a single full-width scrolling column under a top tab-strip: sticky search
 * bar → intro banner → one full-bleed body that swapped between a full-width
 * accordion / a full-width grouped grid / a 2-col tips grid. This file GENUINELY
 * re-architects that into a fresh modern-DOC layout with NEW positions:
 *
 *   1. HEADER ROW   — title (left) + the global search promoted UP INTO the
 *      header band (center/right). The 3-tab top-strip is GONE.
 *   2. KPI STRIP    — a horizontal row of redesign <KpiCard> ledger tiles
 *      (# pagini ghidate / # pași parcurs / # sfaturi) derived from the constant
 *      arrays — content the original never surfaced.
 *   3. DOC BENTO    — a 12-col split that INVERTS the old full-width column:
 *      a narrow LEFT NAV RAIL (4/12: welcome card + a vertical section nav that
 *      replaces the old top tabs + a search-scope hint) and a wide RIGHT CONTENT
 *      PANE (8/12) that renders the active section. Switching sections morphs the
 *      content pane via a shared-element view transition.
 *
 * Behaviour is identical to the original: same `tab` / `search` state, the same
 * "tabs hidden while searching → full-width grouped results" rule (here the nav
 * rail dims its section nav and the content pane shows SearchResults), the same
 * `handleNavigate(page)` deep-link contract with the `window.location.hash`
 * fallback, the same four sub-components (LifecycleTimeline / PagesDirectory /
 * TipsGrid / SearchResults) and `buildSearchIndex`. Same default export name +
 * props signature ({ user, onNavigate }). No native prompt()/confirm() existed,
 * so none to replace.
 */
...
import { filterSearchInputCls, filterSearchIconCls, filterClearInlineBtnCls } from '@/redesign/ui/filterControls';
...
function LifecycleTimeline({ sections, onNavigate }: { sections: TutorialSection[]; onNavigate: (page: string) => void }) {
  ...
  return (
    <div className="rounded-2xl overflow-hidden border border-line/70 stagger-in">
      {sections.map((section, si) => {
        const isOpen = expanded === section.id;
        const Icon = section.icon;

        return (
          <div key={section.id} className="border-b border-line last:border-b-0 bg-surface-secondary">
            <button
              type="button"
                    ...
                    <div key={idx} className={`flex gap-3 px-0 py-3 ${idx < section.steps.length - 1 ? 'border-b border-line' : ''}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {step.tips && step.tips.length > 0 && (
                          <div className="mt-2 rounded-lg bg-accent/5 border-l-2 border-accent/30 px-3 py-1.5">
...
function SearchResults({ hits, query, onNavigate }: { hits: SearchHit[]; query: string; onNavigate: (page: string) => void }) {
  if (hits.length === 0) {
    return (
      <div className="px-5 py-12">
        <EmptyState
          ...
          description="Încearcă alt cuvânt-cheie — căutarea acoperă pagini, parcurs proiect și sfaturi."
        />
      </div>
    );
  }
  ...
  return (
    <div key={query} className="stagger-in">
...
const NAV_ITEMS: { id: Tab; label: string; desc: string; icon: React.ComponentType<{ className?: string }> }[] = [
  ...
  { id: 'pages',     label: 'Toate paginile',  desc: 'Directorul complet de pagini', icon: BookOpen },
  { id: 'tips',      label: 'Sfaturi',          desc: 'Trucuri și scurtături', icon: Lightbulb },
];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 flex-1 min-h-0">
          ...
          <aside className="lg:col-span-4 xl:col-span-3 min-h-0 overflow-y-auto space-y-4 pr-0.5">
            <div className="rounded-2xl bg-accent/5 border border-accent/15 p-4 enter-up">
              ...
              <p className="text-pm-xs text-content-secondary leading-relaxed">
                Automatix gestionează întregul ciclu de viață al unui proiect industrial: de la primul
                contact comercial, prin proiectare și producție, până la livrare și facturare. Folosește
                bara de căutare de sus pentru a găsi rapid o pagină sau un sfat.
              </p>
            </div>
                ...
                return (
                  ...
                  >
                    <span className={`h-8 w-8 rounded-lg flex items-center justify-center shrink-0 ${
                      isActive ? 'bg-accent/15 text-accent' : 'bg-surface-tertiary text-content-muted'
                    }`}>
            ...
            {searching && (
              <div className="rounded-2xl border border-line/70 bg-surface-secondary p-4 enter-up">
                ...
                <p className="text-pm-xs text-content-muted leading-relaxed">
                  Se caută simultan în pagini, parcurs și sfaturi.
                  Golește bara pentru a reveni la secțiuni.
                </p>
              </div>
            )}
          </aside>
          ...
          <section className="lg:col-span-8 xl:col-span-9 min-w-0 min-h-0 overflow-y-auto pr-0.5">
            {searching ? (
              <div
                className="rounded-2xl border border-line/70 bg-surface-primary overflow-hidden vt-morph"
                style={{ viewTransitionName: vtName('tutorial-pane', 'search') } as React.CSSProperties}
              >
                <SearchResults hits={hits} query={search} onNavigate={handleNavigate} />
              </div>
            ) : (
              <div
                className="vt-morph"
