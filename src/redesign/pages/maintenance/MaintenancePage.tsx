...
import { Wrench, Plus, Search, Trash2, Pencil, Loader2, X, Check, AlertCircle, Camera, ImageOff, ListChecks, Boxes, DollarSign, FolderKanban } from '@/icons';
...
import { PageChrome, DashboardLayout, Panel, PAGE_GRID_12, PANEL_HEAD } from '@/app-ui';
import KpiCard from '@/redesign/ui/KpiCard';
import StatusBadge from '@/redesign/ui/StatusBadge';
import { MetricValue, EmptyState } from '@/redesign/ui';
  ...
  return (
    ...
    <DashboardLayout
        ...
        chrome={(
          <PageChrome
            actions={
              <Button size="md" onClick={openCreate} disabled={newDisabled}>
    ...
    >
        {!selectedProject ? (
          <Panel fill className="flex-1 min-h-0" title="Selectează un proiect">
            <EmptyState
              icon={Wrench}
          ...
          <div className={PAGE_GRID_12}>
            <section className="xl:col-span-8 min-w-0 min-h-0 flex flex-col">
              <Panel
                ...
                title="Istoric servisări"
                subtitle={`${visibleServices.length} ${visibleServices.length === 1 ? 'servisare' : 'servisări'}${(search || statusFilter) ? ` din ${services.length}` : ''} · ${pieces.length} piese`}
                bodyClassName="p-0"
              >
                {loading ? (
            ...
            <aside className="xl:col-span-4 space-y-5 min-h-0 overflow-y-auto">
              <Panel padding="md" title="Cost total">
              ...
              <Panel padding="md" title="Distribuție status">
                {services.length === 0 ? (
      ...
      {previewImage && (
        ...
        >
          <img
            src={previewImage}
            alt="Preview"
            className=" max-w-[90vw] max-h-[90vh] object-contain rounded-xl shadow-[var(--elevation-4)]"
  ...
  return (
    ...
    >
        ...
        <div className="flex-1 min-h-0 overflow-y-auto">
          <div ref={dropdownRef} className={`border-b border-line ${PANEL_HEAD}`}>
          ...
          <div className={`border-b border-line ${PANEL_HEAD}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 border-b border-line">
            <div className={`border-r border-line ${PANEL_HEAD}`}>
          ...
          <div className="grid grid-cols-2 border-b border-line">
            ...
            <div className={`flex items-end ${PANEL_HEAD}`}>
...
function PhotoSlot({
  ...
  return (
    <div className={`${PANEL_HEAD} ${borderClass}`}>
