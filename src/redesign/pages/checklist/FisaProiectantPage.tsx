...
import { ClipboardCheck, Save, Loader2, CheckSquare, FileText, Minimize2, Maximize2, Pencil, Eye, CalendarClock, History } from '@/icons';
            ...
            actions={
              checklist ? (
                <div className="flex flex-wrap items-center gap-2">
                  <Button size="md" variant="outline" onClick={() => setCompactView(v => !v)}>
                    {compactView ? <Maximize2 className="h-4 w-4" /> : <Minimize2 className="h-4 w-4" />}
                    {compactView ? 'Vizualizare extinsă' : 'Vizualizare compactă'}
                  </Button>
                  {canEdit && (
                    <Button size="md" variant="outline" onClick={() => void toggleEditMode()}>
                      {editMode ? <Eye className="h-4 w-4" /> : <Pencil className="h-4 w-4" />}
                      {editMode ? 'Vizualizare' : 'Editează'}
                    </Button>
                  )}
                  <Button size="md" variant="outline" onClick={() => void handleSave(false)} disabled={saving}>
                    <Save className="h-4 w-4" /> {saving ? 'Se salvează...' : 'Salvează'}
                  </Button>
                  {canEdit && checklist.status !== 'finalized' && (
                    <Button size="md" onClick={() => void handleFinalize()} disabled={saving}>
                      Finalizează
                    </Button>
                  )}
                </div>
