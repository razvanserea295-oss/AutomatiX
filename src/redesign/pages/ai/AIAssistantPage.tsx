...
import { Send, Bot, User as UserIcon, Loader2, Trash2, Wrench, Zap, Plus, MessageSquare, Sparkles, Activity, Gauge } from 'lucide-react';
...
import StatusDot from '@/redesign/ui/StatusDot';
            ...
            <div className="flex items-center gap-3.5 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <StatusDot tone={connTone} pulse={sending} label={statusLabel} />
                </div>
              </div>
            </div>
            <Button size="md" onClick={handleNewConversation} className="shrink-0">
              <Plus className="h-4 w-4" aria-hidden /> Conversație nouă
            </Button>
          </div>
        </div>

        {}
        <div className="enter-up stagger-in grid grid-cols-2 lg:grid-cols-4 gap-4 shrink-0" style={{ animationDelay: '70ms' }}>
          <KpiCard
            label="Conversații"
            value={totalConvos}
            icon={MessageSquare}
          ...
          <KpiCard
            label="Mesaje în fir"
            value={activeMsgCount}
            icon={Bot}
            hint={active?.title || 'fir nou'}
          />
          <KpiCard
            label="Serviciu AI"
            value={statusLabel}
            icon={Activity}
            iconColor={connTone === 'success' ? 'text-status-green' : connTone === 'danger' ? 'text-status-red' : undefined}
            hint="verificat la 60s"
          />
          <KpiCard
            label="Activitate"
            value={inFlightLabel}
            icon={Gauge}
            iconColor={sending ? 'text-accent' : undefined}
            hint={sending ? `${elapsedSec}s scurse` : 'gata de întrebări'}
          />
        </div>

        {}
        <div className="enter-up grid grid-cols-1 lg:grid-cols-12 gap-4 flex-1 min-h-0" style={{ animationDelay: '140ms' }}>
          ...
          >
            {}
            <div className="flex items-center justify-between gap-3 px-5 py-3.5 border-b border-line/70 shrink-0">
              <div className="flex items-center gap-2.5 min-w-0">
              ...
              {messages.length > 0 && (
                <Button
                  ...
                  className="shrink-0"
                >
                  <Trash2 className="h-3.5 w-3.5" aria-hidden /> Șterge
                </Button>
              )}
            </div>
            ...
            <div ref={chatScrollRef} className="flex-1 min-h-0 min-w-0 overflow-y-auto overflow-x-hidden">
              ...
              {messages.map((msg) =>
                ...
                msg.role === 'user' ? (
                  <div key={msg.id} className="enter-up flex justify-end border-b border-line/30 px-5 py-3.5">
                  ...
                  <div key={msg.id} className="enter-up border-b border-line/30 px-5 py-3.5">
                    <div className="flex max-w-[78%] min-w-0 items-start gap-2.5">
                      <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-accent/20">
              ...
              {sending && (
                <div className="enter-up px-5 py-3.5 border-b border-line/30">
              ...
              {error && (
                <div className="enter-up mx-5 my-3.5 rounded-xl bg-status-red/10 border border-status-red/20 px-3.5 py-3">
