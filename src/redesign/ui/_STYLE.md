...
2. **Sibling primitive imports** → import from `@/redesign/ui/<Sibling>` (NEVER `@/components/ui/...`).
   Any old import from `@/components/ui/...`, `@/components/Modal`, `@/components/FormModal`,
   `@/components/ConfirmDialog` → rewrite the path to `@/redesign/ui/...`.
...
After writing, double-check: same exports? same props? sibling imports use `@/redesign/ui`? No hardcoded hex?
