        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={`relative flex items-center gap-1.5 h-8 px-3 rounded-lg text-pm-2xs font-semibold uppercase tracking-wide transition-smooth duration-150 focus-visible:outline-none focus-visible:shadow-[var(--ring-soft)] ${
                  isActive
        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              disabled={tab.disabled}
              onClick={() => onChange(tab.id)}
              className={`flex items-center gap-1.5 h-8 px-3.5 rounded-full text-pm-xs font-semibold transition-smooth duration-150 focus-visible:outline-none focus-visible:shadow-[var(--ring-soft)] ${
                isActive
                  ? 'bg-accent-muted text-accent shadow-[var(--elevation-1)]'
                  : 'text-content-secondary hover:text-content-primary hover:bg-surface-tertiary disabled:opacity-40 disabled:cursor-not-allowed'
              }`}
            ...
            >
              ...
              <span>{tab.label}</span>
              {tab.count != null && (
                <span className={`tabular-nums text-pm-2xs ${isActive ? 'text-accent/80' : 'text-content-muted/70'}`}>
                  {tab.count}
                </span>
              )}
            </button>
      ...
      {tabs.map((tab) => {
        ...
        return (
          <button
            ...
            onClick={() => onChange(tab.id)}
            aria-pressed={isActive}
            className={`group relative flex items-center gap-2 px-4 h-10 text-pm-base font-medium transition-smooth duration-150 focus-visible:outline-none focus-visible:shadow-[var(--ring-soft)] rounded-t-lg ${
              isActive
                ? 'text-accent'
                : 'text-content-muted hover:text-content-primary hover:bg-surface-secondary/60 disabled:opacity-40 disabled:cursor-not-allowed'
            }`}
          >
            {tab.icon}
