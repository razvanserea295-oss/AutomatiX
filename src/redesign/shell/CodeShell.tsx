...
// Map a notification "kind" to a severity dot colour (VS Code Problems-style).
...
export default function CodeShell({
  ...
  // Refresh the Problems list whenever the panel is opened.
  ...
  // Live status-bar clock (setInterval is fine while hidden — unlike rAF).
  useEffect(() => {
  ...
  // Ctrl+B toggles the side bar, Ctrl+J the bottom panel (VS Code defaults).
  useEffect(() => {
  ...
  // Explorer tree rows = the active workspace's subpages (Sistem trimmed for
  // non-admins, matching the SaaS rule). Direct pages (dashboard/alerts/chat)
  // have no subpages → the tree shows the active item itself.
  ...
  return (
    <div className="vsc-root flex flex-col h-dvh-app w-full overflow-hidden">
      {/* Title bar — functional controls only (no decorative menus). ---- */}
      <header className="vsc-titlebar">
        ...
        <span className="vsc-menu font-semibold" style={{ color: '#fff' }}>automatiX</span>
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
        ...
        <main className="vsc-editor relative isolate flex flex-1 flex-col min-h-0 min-w-0 overflow-hidden">
          <div className="vsc-editor-content flex flex-1 flex-col min-h-0 overflow-hidden">
            {children}
          </div>

          {/* Bottom panel — Probleme / Activitate (operations-focused). */}
          {panelOpen && (
      ...
      <footer className="vsc-statusbar">
        ...
        <span className="vsc-status-item">{clock}</span>
