...
export default function ChatPage({ user }: ChatPageProps) {
  ...
  const removeMember = async (memberId: number, memberName: string) => {
    if (!groupSettings) return;
    if (!window.confirm(`Sigur vrei să elimini "${memberName}" din grup?`)) return;
    try {
  ...
  return (
    <Page className="!overflow-hidden">
      ...
      {}
      <div className="w-72 border-r border-line flex flex-col shrink-0 min-h-0 bg-surface-secondary">
        {}
        <div className="px-3 py-2.5 border-b border-line shrink-0">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-semibold text-content-primary">Mesaje</h2>
            <div className="flex gap-1">
              <button onClick={() => setShowNewGroup(true)} title="Grup nou"
                className="p-1.5 text-content-muted hover:bg-surface-tertiary hover:text-accent">
                <UsersIcon className="h-3.5 w-3.5" />
              </button>
              <button onClick={() => setShowNewChat(true)} title="Conversatie noua"
                className="h-7 w-7 flex items-center justify-center bg-accent text-surface-primary">
        ...
        <div className="flex-1 min-h-0 overflow-y-auto">
            ...
            return (
              <button key={c.id} onClick={() => setActiveConvo(c.id)}
                className={`w-full flex items-center gap-2.5 px-3 py-2.5 text-left transition-colors border-b border-line/40 ${
                  active ? 'bg-accent/5 border-l-2 border-l-accent' : 'hover:bg-surface-tertiary'
                }`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1 mt-0.5">
                    ...
                    {c.unread_count > 0 && (
                      <span className="flex items-center justify-center h-4 min-w-[16px] px-1 rounded-full bg-accent text-surface-primary text-pm-2xs font-bold tabular-nums">{c.unread_count}</span>
                    )}
                  </div>
                </div>
              </button>
          ...
          {filtered.length === 0 && (
            <div className="p-6 text-center text-xs text-content-muted">
              {convos.length === 0 ? 'Nicio conversatie' : 'Niciun rezultat'}
            </div>
          )}
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden min-w-0 min-h-0 relative"
        ...
        {!activeConvo ? (
          <div className="flex-1 flex items-center justify-center text-content-muted bg-surface-primary">
            <div className="text-center">
              <MessageCircle className="h-10 w-10 mx-auto mb-2 opacity-20" />
              <p className="text-sm">Selectează o conversatie</p>
            </div>
          </div>
        ) : (
          <>
            {}
            <div className="h-12 px-4 border-b border-line flex items-center gap-3 shrink-0 bg-surface-secondary">
              <button onClick={() => setActiveConvo(null)} className="lg:hidden p-1.5 text-content-muted hover:bg-surface-tertiary hover:text-accent">
              ...
              <button
                ...
                className={`flex items-center gap-3 min-w-0 ${
                  activeConvoData?.is_group ? 'hover:bg-surface-tertiary px-1 -mx-1 transition-colors cursor-pointer' : 'cursor-default'
                }`}
                title={activeConvoData?.is_group ? 'Detalii grup' : undefined}
              >
                {activeConvoData?.is_group && headerGroupAvatar ? (
                  <img
                    src={headerGroupAvatar}
                    alt={activeConvoData.group_name || 'Grup'}
                  ...
                  <div className={`h-8 w-8 rounded-full flex items-center justify-center text-pm-2xs font-bold text-surface-primary shrink-0 ${
                    ...
                    {activeConvoData?.is_group ? <UsersIcon className="h-3.5 w-3.5" /> : getInitials(activeConvoData?.other_user_name || '?')}
                  </div>
                )}
                <div className="text-left min-w-0">
                  <p className="text-xs font-semibold text-content-primary truncate">
                    {activeConvoData?.is_group ? activeConvoData.group_name : activeConvoData?.other_user_name}
                  </p>
                  <p className="text-pm-2xs text-content-muted">
                    {activeConvoData?.is_group ? 'Grup · click pentru detalii' : activeConvoData?.other_user_role}
                  </p>
                </div>
              </button>
            </div>
              ...
              {loadingMsgs && messages.length === 0 && (
                <div className="space-y-2 animate-pulse" aria-hidden>
                  <div className="flex justify-start">
                    <div className="bg-surface-secondary border border-line px-3 py-2 w-44 h-9" />
                  </div>
                  <div className="flex justify-end">
                    <div className="bg-accent/10 px-3 py-2 w-56 h-9" />
                  </div>
                  <div className="flex justify-start">
                    <div className="bg-surface-secondary border border-line px-3 py-2 w-32 h-9" />
                  </div>
                  <div className="flex justify-end">
                    <div className="bg-accent/10 px-3 py-2 w-40 h-12" />
                  </div>
                </div>
              )}
                    ...
                    <div className={`px-3 py-2 ${
                      ...
                      {m.message_type === 'file' && m.attachment_name && isImage(m.attachment_name) && m.attachment_data && (
                        <button onClick={() => setPreviewImage(`data:image/${m.attachment_name!.split('.').pop()};base64,${m.attachment_data}`)}
                          className="block mb-1.5 overflow-hidden hover:opacity-90 transition-opacity">
                          <img src={`data:image/${m.attachment_name.split('.').pop()};base64,${m.attachment_data}`}
                            alt={m.attachment_name} loading="lazy" decoding="async" className="max-w-[280px] max-h-[200px] object-cover" />
                        </button>
                      )}
                      ...
                      {m.message_type === 'file' && m.attachment_name && !isImage(m.attachment_name) && m.attachment_data && (
                        <button onClick={() => downloadBlob(m.attachment_data!, m.attachment_name!)}
                          className={`flex items-center gap-2 mb-1.5 px-2.5 py-2 border w-full ${
                      ...
                      {(m.message_type === 'report' || m.message_type === 'link') && m.reference_label && (
                        <div className="flex items-center gap-2 mb-1.5 px-2 py-1.5 bg-accent/5">
            ...
            {}
            {dragging && (
              <div className="absolute inset-0 bg-accent/5 border-2 border-dashed border-accent/30 flex items-center justify-center z-10 pointer-events-none">
                <div className="text-center">
            ...
            {pendingFiles.length > 0 && (
              <div className="px-4 py-2 border-t border-line flex flex-wrap gap-1.5 shrink-0 bg-surface-secondary">
                {pendingFiles.map((f, i) => (
                  <div key={i} className="flex items-center gap-1.5 px-2 py-1 bg-surface-tertiary text-xs text-content-primary">
            ...
            <div className="px-4 py-3 border-t border-line bg-surface-secondary shrink-0 flex items-end gap-2">
              <button onClick={() => fileInputRef.current?.click()}
                className="p-1.5 text-content-muted hover:bg-surface-tertiary hover:text-accent shrink-0">
              ...
              <textarea
                ...
                className="flex-1 min-h-[36px] max-h-[120px] resize-none bg-surface-primary border border-line text-xs text-content-primary placeholder:text-content-muted focus:outline-none focus:ring-1 focus:ring-accent/60 px-3 py-2"
              />
              <Button
                size="sm"
      ...
      {}
      {showNewChat && (
        <aside className="absolute right-0 top-0 bottom-0 z-30 w-full sm:w-[400px] bg-surface-secondary border-l border-line shadow-xl flex flex-col">
          <div className="p-3 border-b border-line flex items-center justify-between shrink-0">
            <h3 className="text-xs font-semibold text-content-primary">Conversatie noua</h3>
            <button onClick={() => setShowNewChat(false)} className="p-1.5 text-content-muted hover:bg-surface-tertiary hover:text-accent"><X className="h-4 w-4" /></button>
          </div>
          <div className="flex-1 min-h-0 overflow-y-auto">
            {allUsers.map(u => (
      ...
      {}
      {showNewGroup && (
        <aside className="absolute right-0 top-0 bottom-0 z-30 w-full sm:w-[440px] bg-surface-secondary border-l border-line shadow-xl flex flex-col">
            <div className="p-3 border-b border-line flex items-center justify-between shrink-0">
              <h3 className="text-xs font-semibold text-content-primary">Grup nou</h3>
              <button onClick={() => setShowNewGroup(false)} className="p-1.5 text-content-muted hover:bg-surface-tertiary hover:text-accent"><X className="h-4 w-4" /></button>
            </div>
            <div className="p-3 border-b border-line shrink-0">
              <input value={groupName} onChange={e => setGroupName(e.target.value)} placeholder="Numele grupului..."
                className="w-full h-9 border border-line bg-surface-primary px-3 text-xs text-content-primary placeholder:text-content-muted focus:outline-none focus:ring-1 focus:ring-accent/60" />
            </div>
            <div className="flex-1 min-h-0 overflow-y-auto">
              {allUsers.map(u => (
                <label key={u.id} className="flex items-center gap-2.5 px-3 py-2 hover:bg-surface-tertiary cursor-pointer border-b border-line/40">
                  <input type="checkbox" checked={groupMembers.includes(u.id)}
                    ...
                    className="h-3.5 w-3.5 border border-line accent-[var(--color-accent)]" />
      ...
      {(groupSettings || groupSettingsLoading) && (
        <aside className="absolute right-0 top-0 bottom-0 z-30 w-full sm:w-[440px] bg-surface-secondary border-l border-line shadow-xl flex flex-col">
          <div className="p-3 border-b border-line flex items-center justify-between shrink-0">
            <h3 className="text-xs font-semibold text-content-primary">Detalii grup</h3>
            <button onClick={closeGroupSettings} className="p-1.5 text-content-muted hover:bg-surface-tertiary hover:text-accent">
              <X className="h-4 w-4" />
            </button>
          </div>

          {groupSettingsLoading && !groupSettings ? (
            <div className="flex-1 flex items-center justify-center text-pm-xs text-content-muted">
              Se încarcă...
            </div>
          ) : groupSettings ? (
            !showAddMembers ? (
              <div className="flex-1 min-h-0 overflow-y-auto">
                  ...
                  {editingGroupName && isGroupAdmin ? (
                    <div className="w-full flex items-center gap-1.5">
                      <input
                        ...
                        className="flex-1 h-8 border border-line bg-surface-primary px-2.5 text-sm text-content-primary focus:outline-none focus:ring-1 focus:ring-accent/60"
                      />
                ...
                <ul>
                  {groupSettings.members.map(m => (
                    <li key={m.user_id} className="flex items-center gap-2.5 px-3 py-2 border-b border-line/40">
                      <div className={`h-8 w-8 rounded-full flex items-center justify-center text-pm-2xs font-bold text-surface-primary shrink-0 ${
                      ...
                      {isGroupCreator && !m.is_creator && (
                        <div className="flex items-center gap-1 shrink-0">
                          <button
                            ...
                            className={`p-1.5 transition-colors ${m.is_admin ? 'text-status-blue hover:text-status-amber' : 'text-content-muted hover:text-status-blue'}`}
                          >
                          ...
                          <button
                            ...
                            className="p-1.5 text-content-muted hover:text-status-red"
                          >
      ...
      {previewImage && (
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center cursor-pointer" onClick={() => setPreviewImage(null)}>
          <img src={previewImage} alt="Preview" className="max-w-[90vw] max-h-[90vh] object-contain shadow-2xl" />
