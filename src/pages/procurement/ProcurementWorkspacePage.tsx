...
import { Pencil, Trash2, Loader2, Download, Printer, Plus, Package, Truck, Clock, CheckCircle2 } from 'lucide-react';
...
function SuppliersTab() {
  ...
  if (loading) {
    return (
      <div className="flex flex-1 flex-col" aria-busy="true">
        ...
        <div className="border-b border-line bg-surface-secondary overflow-hidden">
  ...
  return (
    <div>
      ...
      <SupplierToolsBar suppliers={suppliers.map(s => ({ id: s.id, name: s.name }))} />

      <div className="flex items-center justify-between px-6 py-2.5 border-b border-line bg-surface-primary">
        <span className="text-xs text-content-muted">{suppliers.length} furnizori</span>
          ...
          {suppliers.length > 0 && (
            <>
              <button
                ...
                className="flex items-center gap-1.5 border border-line px-2.5 py-1 text-pm-2xs font-semibold text-content-secondary hover:bg-surface-tertiary transition-colors"
              >
                <Download className="h-3 w-3" /> Export CSV
              </button>
              <button
                type="button"
                onClick={() => printSuppliers(suppliers)}
                className="flex items-center gap-1.5 border border-line px-2.5 py-1 text-pm-2xs font-semibold text-content-secondary hover:bg-surface-tertiary transition-colors"
              >
                <Printer className="h-3 w-3" /> Printeaza
              </button>
            </>
          )}
          <button
            type="button"
            onClick={() => openModal()}
            className="h-8 flex items-center gap-1.5 bg-accent px-3.5 text-xs font-semibold text-surface-primary hover:opacity-90 transition-opacity"
          >
      ...
      <div className="bg-surface-secondary overflow-auto max-h-[70vh]">
          ...
          <tbody>
            {sortedSuppliers.length === 0 ? (
              <tr>
                <td colSpan={9} className="p-0">
                  <EmptyState
                    ...
                    body="Adaugă primul furnizor pentru a putea genera comenzi de achiziție."
                    actionLabel="Adaugă furnizor"
                    onAction={() => openModal()}
                  />
                </td>
              </tr>
              ...
              sortedSuppliers.map((s) => (
                ...
                >
                  <td className="px-3 py-2.5 text-sm font-medium text-content-primary">
                  ...
                  <td className="px-3 py-2.5 text-xs text-content-secondary truncate" title={categoryLabel(s.category)}>{categoryLabel(s.category)}</td>
                  <td className="px-3 py-2.5 text-xs text-content-secondary">
                    <span className="line-clamp-2" title={s.products ?? ''}>{s.products ?? '—'}</span>
                  </td>
                  <td className="px-3 py-2.5 text-xs text-content-secondary truncate" title={s.contact_person ?? ''}>{s.contact_person ?? '—'}</td>
                  <td className="px-3 py-2.5 text-xs text-content-secondary truncate" title={s.email ?? ''}>{s.email ?? '—'}</td>
                  <td className="px-3 py-2.5 text-xs text-content-secondary font-mono truncate" title={s.phone ?? ''}>{s.phone ?? '—'}</td>
                  <td className="px-3 py-2.5 text-xs text-content-secondary font-mono truncate" title={s.cui ?? ''}>{s.cui ?? '—'}</td>
                  <td className="px-3 py-2.5 text-xs text-content-secondary truncate" title={s.payment_terms ?? ''}>{s.payment_terms ?? '—'}</td>
...
function PurchaseOrdersTab() {
  ...
  return (
    <div>
      <div className="flex items-center justify-between px-6 py-2.5 border-b border-line bg-surface-primary">
        <span className="text-xs text-content-muted">{orders.length} comenzi</span>
        <div className="flex items-center gap-2">
          {orders.length > 0 && (
            <button
              type="button"
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="border-b border-line">
              <th className="px-4 py-2.5 text-pm-2xs font-bold uppercase tracking-[0.14em] text-content-muted">Nr. comanda</th>
              <th className="px-4 py-2.5 text-pm-2xs font-bold uppercase tracking-[0.14em] text-content-muted">Furnizor</th>
              <th className="px-4 py-2.5 text-pm-2xs font-bold uppercase tracking-[0.14em] text-content-muted">Data</th>
              <th className="px-4 py-2.5 text-pm-2xs font-bold uppercase tracking-[0.14em] text-content-muted">Status</th>
              <th className="px-4 py-2.5 text-pm-2xs font-bold uppercase tracking-[0.14em] text-content-muted">Total</th>
            </tr>
          </thead>
          <tbody>
            {orders.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-4 py-8 text-center text-xs text-content-muted">
                  Nu există comenzi de achizitie.
                </td>
              </tr>
            ) : (
              orders.map((o) => (
                ...
                >
                  <td className="px-4 py-2.5 text-sm font-medium text-content-primary font-mono">{o.order_number}</td>
                  <td className="px-4 py-2.5 text-xs text-content-secondary">{o.supplier_name}</td>
                  <td className="px-4 py-2.5 text-xs text-content-muted">{formatDate(o.order_date)}</td>
                  <td className="px-4 py-2.5">
                    <StatusBadge tone={orderStatusTone(o.status)} label={o.status} size="xs" />
                  </td>
                  <td className="px-4 py-2.5 text-sm font-medium text-content-primary tabular-nums">{formatCurrency(o.total)}</td>
                </tr>
              ))
            )}
...
function PurchaseOrderCreateModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div className="bg-surface-secondary border border-line shadow-xl w-full max-w-2xl max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="px-5 py-3 border-b border-line flex items-center justify-between">
          <h3 className="text-sm font-semibold text-content-primary">Comanda noua de achizitie</h3>
          <button onClick={onClose} className="text-content-muted hover:text-content-primary">×</button>
        </div>
        <div className="p-5 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-pm-2xs font-semibold uppercase tracking-wide text-content-muted mb-1 block">Furnizor *</label>
            ...
            <div>
              <label className="text-pm-2xs font-semibold uppercase tracking-wide text-content-muted mb-1 block">Proiect *</label>
          ...
          <div>
            <label className="text-pm-2xs font-semibold uppercase tracking-wide text-content-muted mb-1 block">Referinta interna</label>
            <input value={internalRef} onChange={e => setInternalRef(e.target.value)} placeholder="ex: PO-2026-001"
              className="w-full border border-line bg-surface-primary px-2 py-1.5 text-xs" />
          </div>
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-pm-2xs font-semibold uppercase tracking-wide text-content-muted">Articole *</label>
              ...
              {lines.map((l, i) => (
                <div key={i} className="flex items-center gap-2">
                  <select value={l.material_id ?? ''} onChange={e => updateLine(i, { material_id: e.target.value ? Number(e.target.value) : null })}
                    className="flex-1 border border-line bg-surface-primary px-2 py-1.5 text-xs">
                  ...
                  <input type="number" step="0.01" min="0.01" value={l.qty}
                    ...
                    className="w-24 border border-line bg-surface-primary px-2 py-1.5 text-xs tabular-nums" />
                  {lines.length > 1 && (
                    <button type="button" onClick={() => removeLine(i)} className="text-status-red hover:opacity-70 px-2">×</button>
                  )}
                </div>
              ))}
            </div>
        ...
        <div className="px-5 py-3 border-t border-line flex items-center justify-end gap-2">
          <button onClick={onClose} className="border border-line px-3 py-1.5 text-xs text-content-secondary hover:bg-surface-tertiary">Anulează</button>
          <button onClick={handleSubmit} disabled={saving}
            className="bg-accent px-4 py-1.5 text-xs font-semibold text-surface-primary hover:opacity-90 disabled:opacity-50">
      ...
      <div className="w-80 shrink-0 bg-surface-secondary border-r border-line overflow-hidden flex flex-col min-h-0">
        ...
        <div className="flex-1 min-h-0 overflow-y-auto">
          {pendingOrders.length === 0 ? (
            <p className="text-xs text-content-muted text-center py-8">Nicio comanda in asteptare</p>
          ) : (
            pendingOrders.map(o => (
              <button
                key={o.id}
              ...
              >
                <div className="text-sm font-medium text-content-primary font-mono">{o.order_number}</div>
                <div className="text-xs text-content-muted mt-0.5">{o.supplier_name}</div>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto bg-surface-primary">
        {!selectedPO ? (
          <div className="flex flex-1 items-center justify-center h-full">
            <p className="text-xs text-content-muted">Selectează o comanda pentru receptie</p>
          </div>
        ) : loadingDetail ? (
          <div className="flex flex-1 items-center justify-center h-full">
          ...
          <div>
            ...
            <div className="bg-surface-secondary border-b border-line px-4 py-3">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-sm font-semibold text-content-primary font-mono">{selectedPO.order_number}</h3>
                  <p className="text-xs text-content-muted mt-0.5">{selectedPO.supplier_name} &middot; {formatDate(selectedPO.order_date)}</p>
                </div>
                    ...
                    selectedPO.lines.map(line => {
                      ...
                      return (
                        <tr key={line.id} className={`border-b border-line ${isComplete ? 'opacity-50' : 'hover:bg-surface-tertiary/30'} transition-colors`}>
                          <td className="px-4 py-2.5">
                            <div className="text-xs text-content-primary font-medium">{line.material_name}</div>
                            <div className="text-pm-2xs text-content-muted font-mono">{line.material_code}</div>
                          </td>
                          <td className="px-4 py-2.5 text-xs text-content-primary text-right tabular-nums">{line.qty_ordered} {line.unit}</td>
                          ...
                          <td className="px-4 py-2.5">
                            ...
                            {isComplete && (
                              <span className="text-pm-2xs text-status-green font-semibold">Complet</span>
                            )}
                          </td>
                        </tr>
                      );
    ...
    <Page className="mod-shell">
      <div className="mod-canvas">
        ...
        <HeroHeader
          ...
          icon={Package}
          title="Achiziții"
          subtitle="Furnizori, comenzi de achiziție și recepții pentru proiecte"
        />

        {}
        <div className="mod-kpis enter-up" style={{ animationDelay: '80ms' }}>
          <KpiMini icon={Truck}        label="Furnizori"      value={kpi.suppliers} />
          <KpiMini icon={Package}      label="Comenzi"        value={kpi.orders} />
          <KpiMini icon={Clock}        label="În așteptare"   value={kpi.pending} warn={kpi.pending > 0} />
          <KpiMini icon={CheckCircle2} label="Finalizate"     value={kpi.done} />
        </div>
