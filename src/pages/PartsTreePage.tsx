...
import { ChevronRight, ChevronDown, Search, FolderTree, FileBox, Upload, FolderUp, Sparkles, Loader2, ChevronsUpDown, Plus, Wand2, Tags, Download, Boxes, X, Trash2 } from 'lucide-react';
import SupplierCodesModal from './parts-tree/SupplierCodesModal';
...
import type { PartTreeNode } from './parts-tree/types';
import { catColor, formatSize, countTotal } from './parts-tree/types';
...
import { filterSearchInputCls, filterSearchIconCls, filterSelectCls } from '@/components/ui/filterControls';
...
function TreeRowItemBase({
  index, style,
  rows, selectedId, collapsed, searchQuery, onRowClick, onToggleCollapse,
    ...
    >
      ...
      <div className="w-5 h-5 flex items-center justify-center shrink-0">
        {row.hasChildren ? (
          ...
          >
            {isCollapsed ? <ChevronRight className="h-3 w-3 text-content-muted" /> : <ChevronDown className="h-3 w-3 text-content-muted" />}
          </button>
        ) : <span className="w-4" />}
      </div>
      ...
      <span className={`text-xs truncate flex-1 ${isSelected ? 'text-content-primary font-semibold' : 'text-content-primary font-medium'}`}>
        {nameEl}
      </span>
      ...
      {row.node.supplier_code && (
        <span
          className="ml-1 text-pm-2xs px-1.5 py-0.5 rounded font-mono font-bold text-white shrink-0"
...
export default function PartsTreePage({ initialProjectId }: PartsTreePageProps) {
  ...
  const handleWipeTree = useCallback(async () => {
    ...
    const ok = confirm(
      `ȘTERGI întreg arborele de piese din "${projName}"?\n\n` +
      ...
      `Această acțiune nu poate fi anulată. Continui?`,
    );
    if (!ok) return;
    try {
  ...
  }), [filteredRows, selectedId, collapsed, debouncedSearch, handleRowClick, toggleCollapse]);
      ...
      <div className="flex-1 min-h-0 flex flex-col overflow-y-auto">
        ...
        <div className="px-5 pt-4 pb-8 space-y-4 shrink-0">
          ...
          <div className="mod-kpis enter-up" style={{ animationDelay: '80ms' }}>
            <KpiMini icon={FolderTree} label="Total fișiere" value={totalParts} />
            <KpiMini icon={Boxes}      label="Ansambluri"    value={assembliesTotal} />
            <KpiMini icon={FileBox}    label="Piese proiect" value={pieces.length} />
            <KpiMini icon={Search}     label="Vizibile"      value={filteredRows.length} />
          </div>
        </div>
        ...
        <div className="shrink-0 flex items-center gap-2.5 mx-5 mt-1 mb-2 rounded-xl px-4 min-h-14 py-2.5 glass-surface flex-wrap">
          <FolderTree className="h-4 w-4 text-content-muted shrink-0" />
          <select value={selectedProject ?? ''} onChange={e => setSelectedProject(Number(e.target.value) || null)}
          ...
          <div className="relative group">
            ...
            <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Caută piesa..."
              className={`${filterSearchInputCls} !w-56`} />
          </div>

          <button onClick={collapsed.size > 0 ? expandAll : collapseAll}
            className="border border-line px-2.5 py-1 text-pm-2xs font-semibold text-content-secondary hover:bg-surface-tertiary flex items-center gap-1">
          ...
          <button
            ...
            className={`border px-2.5 py-1 text-pm-2xs font-semibold flex items-center gap-1 transition-colors ${
              assemblyOnly
          ...
          <div className="flex items-center gap-2">
            {LEGEND.map(l => (
              <span key={l.role} className="flex items-center gap-1 text-pm-2xs text-content-muted">
                <span className="h-2.5 w-2.5" style={{ backgroundColor: ROLE_COLORS[l.role] }} />
                {l.label}
              </span>
            ))}
          </div>
          ...
          <div className="flex items-center gap-1.5">
            <Button variant="outline" size="sm" onClick={() => setCodesModalOpen(true)} title="Coduri furnizor — legenda">
              <Tags className="h-3.5 w-3.5" /> Coduri
            </Button>
            {selectedProject && (
              <>
                <Button size="sm" onClick={() => openAddModal()} disabled={stages.length === 0}>
                ...
                <button
                  ...
                  className="p-1.5 text-content-muted hover:bg-surface-tertiary hover:text-accent disabled:opacity-50 transition-colors"
                >
                  {aiLoading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Sparkles className="h-3.5 w-3.5" />}
                </button>
              </>
            )}
            <span className="text-pm-2xs text-content-muted tabular-nums" title={
              assemblyOnly
        ...
        {showFolderInput && (
          <div className="shrink-0 flex gap-2.5 items-center px-5 py-2 border-b border-line bg-surface-secondary">
            <input value={folderPath} onChange={e => setFolderPath(e.target.value)} placeholder="C:\Proiecte\Piese"
              className="flex-1 h-8 border border-line bg-surface-primary px-3 text-xs font-mono text-content-primary placeholder:text-content-muted focus:outline-none" />
        ...
        {}
        {status && (
          <div className={`border-b border-line px-4 py-2 text-xs ${status.type === 'ok' ? 'bg-status-green/10 text-status-green' : 'bg-status-red/10 text-status-red'}`}>
            {status.msg}
          </div>
        )}
        {aiInsight && (
          <div className="border-b border-line bg-accent/5 px-4 py-2">
            <span className="text-pm-2xs font-medium uppercase tracking-wider text-accent mr-2">AI</span>
            <span className="text-xs text-content-secondary">{aiInsight}</span>
          </div>
        )}
        ...
        <div className="flex h-[70vh] min-h-[400px] shrink-0 overflow-hidden">
          {}
          <div
            ref={scrollRef}
              ...
              <div className="flex h-full items-center justify-center p-8">
                <GlassCard size="regular" className="enter-up w-full max-w-md flex flex-col items-center gap-3 px-8 py-12 text-center">
                  <span className="h-16 w-16 rounded-2xl bg-accent/10 flex items-center justify-center">
                    <FileBox className="h-8 w-8 text-accent/70" />
                  </span>
                  <div>
                    <p className="text-pm-md font-semibold text-content-primary">
                      {selectedProject ? 'Niciun fișier importat' : 'Selectează un proiect'}
                    </p>
                  ...
                  {selectedProject && (
                    <Button size="sm" onClick={handlePickFolder} className="mt-1">
          ...
          {selectedNode && (
            <div className="w-72 shrink-0 border-l border-line bg-surface-secondary overflow-y-auto flex flex-col shadow-[-4px_0_6px_-2px_rgba(0,0,0,0.08)]">
              {
}
              <div className="border-b border-line px-4 py-3 flex items-center gap-2 sticky top-0 bg-surface-secondary z-10">
                <span className="h-3.5 w-3.5 shrink-0" style={{ backgroundColor: catColor(selectedNode.category) }} />
                <span className="font-medium text-sm text-content-primary truncate flex-1">{selectedNode.name}</span>
                <button
                  type="button"
                  onClick={() => setSelectedId(null)}
                  title="Închide detaliile"
                  aria-label="Închide detaliile"
                  className="h-6 w-6 shrink-0 flex items-center justify-center text-content-muted hover:text-content-primary hover:bg-surface-tertiary rounded transition-colors"
                >
              ...
              <div className="flex flex-col">
                {[
                  ...
                  <div key={label} className="border-b border-line px-4 py-2 flex justify-between items-center">
                    ...
                    <span className="text-xs text-content-primary font-medium capitalize">{val}</span>
                  </div>
                ))}
              </div>
