...
const NAV_ITEMS: { id: Section; label: string; icon: typeof Settings; description: string }[] = [
  { id: 'aspect',     label: 'Aspect',             icon: Sun,        description: 'Temă și personalizare' },
  { id: 'notificari', label: 'Notificări',         icon: Bell,       description: 'Email și in-app' },
  { id: 'cont',       label: 'Cont',               icon: UserIcon,   description: 'Profil utilizator' },
  { id: 'email',      label: 'Email',              icon: Mail,       description: 'IMAP / SMTP' },
  { id: 'fiscal',     label: 'Fiscal',             icon: Building2,  description: 'Date companie' },
  { id: 'anunturi',   label: 'Anunțuri',           icon: Megaphone,  description: 'Popup-uri pentru toți userii' },
  { id: 'mentenanta', label: 'Mod mentenanță',     icon: Wrench,     description: 'Blochează accesul non-admin' },
  { id: 'server',     label: 'Server',             icon: Server,     description: 'Conexiune rețea' },
  { id: 'ai',         label: 'AI Service',         icon: Bot,        description: 'Motor inteligență' },
  { id: 'audit',      label: 'Jurnal modificări',  icon: ScrollText, description: 'Audit log' },
  { id: 'backup',     label: 'Backup',             icon: Database,   description: 'Copie de siguranță DB' },
  { id: 'despre',     label: 'Despre',             icon: Info,       description: 'Versiune și licență' },
  { id: 'ajutor',     label: 'Ajutor',             icon: HelpCircle, description: 'Ghid, FAQ și changelog' },
];
...
export default function SettingsPage({ user, onThemeChange, currentTheme }: SettingsPageProps) {
  ...
  const ActiveIcon = visibleNav.find(n => n.id === activeSection)?.icon || Settings;

  return (
    <Page className="mod-shell">
      <Page.Body maxWidth="full" padding="flush" className="!overflow-hidden">
        <div className="flex flex-col h-full min-h-0">
          ...
          {}
          <div className="flex flex-1 min-h-0 mx-5 mb-5 rounded-2xl glass-surface overflow-hidden enter-up" style={{ animationDelay: '120ms' }}>
            {}
            <nav className="w-[220px] shrink-0 border-r border-line/60 overflow-y-auto py-2" aria-label="Secțiuni setări">
              {visibleNav.map((item) => {
                ...
                return (
                  ...
                  >
                    {active && <span className="absolute left-0 top-1 bottom-1 w-[3px] rounded-r-full bg-accent" />}
                    <Icon className={`h-4 w-4 shrink-0 ${active ? 'text-accent' : 'text-content-muted'}`} />
                    <div className="min-w-0 flex-1">
                      <p className={`text-pm-sm truncate ${active ? 'font-semibold' : 'font-medium'}`}>{item.label}</p>
                    </div>
                  </button>
                );
              })}
            ...
            {}
            <div className="flex-1 min-w-0 overflow-y-auto py-5 px-6">
              <SetupContinueBanner isAdmin={(user?.role_name || '').toLowerCase() === 'admin'} />
              {}
              <div className="flex items-center gap-3 mb-6 pb-4 border-b border-line/60">
                <div className="h-9 w-9 rounded-lg bg-accent/8 flex items-center justify-center">
                  <ActiveIcon className="h-4.5 w-4.5 text-accent" />
                </div>
                <div>
                  ...
                  <p className="text-pm-xs text-content-muted">
                    {visibleNav.find(n => n.id === activeSection)?.description}
                  </p>
                </div>
              </div>
...
function SetupContinueBanner({ isAdmin }: { isAdmin: boolean }) {
  ...
  return (
    <div className="mb-5 flex items-center gap-3 rounded-lg border border-accent/30 bg-accent/8 px-4 py-3">
      <Building2 className="h-4 w-4 text-accent shrink-0" />
      <div className="flex-1 min-w-0">
        <p className="text-pm-sm font-semibold text-content-primary">Configurarea inițială nu este finalizată</p>
        <p className="text-pm-xs text-content-muted">Completează datele firmei, logo-ul și setările fiscale pentru documentele oficiale.</p>
      </div>
      <button
        onClick={openWizard}
        className="inline-flex items-center gap-1.5 h-9 px-3 rounded-lg bg-accent text-on-accent text-pm-xs font-semibold hover:opacity-90 transition-colors shrink-0"
      >
        ...
        <table className="w-full">
          ...
          <tbody>
            {prefs.map(p => {
              ...
              return (
                <tr key={p.event_type} className="border-t border-line/50 hover:bg-surface-primary/50 transition-colors">
...
const KIND_COLOR: Record<BackupItem['kind'], string> = {
  rolling:      'bg-status-green/10 text-status-green',
  'pre-migrate':'bg-status-blue/10 text-status-blue',
  manual:       'bg-accent/10 text-accent',
  other:        'bg-content-muted/10 text-content-muted',
};
  ...
  return (
    <div className="space-y-6">
      ...
      <SectionGroup title={`Backup-uri (${list.length})`}>
        {list.length === 0 ? (
          <p className="text-pm-sm text-content-muted">Nu există încă niciun backup.</p>
        ) : (
                ...
                {list.map((b) => (
                  <tr key={b.name} className="border-t border-line/60 hover:bg-surface-tertiary/30">
                    ...
                    <td className="px-3 py-2">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded text-pm-2xs font-medium ${KIND_COLOR[b.kind]}`}>
                        {KIND_LABEL[b.kind]}
                      </span>
                    </td>
