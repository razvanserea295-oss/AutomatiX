...
import { Pencil, Trash2, Plus, Shield, Loader2, LayoutDashboard, Users, UserCheck, KeyRound } from 'lucide-react';
...
const ROLE_LABELS: Record<string, string> = {
  admin: 'Admin', manager: 'Manager', user: 'Utilizator',
    ...
    <Page className="mod-shell !overflow-hidden">
      ...
      <div className="px-5 pt-4 pb-8 shrink-0 space-y-4">
        <HeroHeader
          ...
          icon={Users}
          title="Utilizatori"
          subtitle="Conturi, roluri și acces pe pagini"
          actions={
            <button
              onClick={() => openModal()}
              className="h-8 bg-accent px-3.5 text-xs font-semibold text-surface-primary hover:bg-accent/90 flex items-center gap-1 transition-colors rounded-md"
            >
        ...
        <div className="mod-kpis enter-up" style={{ animationDelay: '80ms' }}>
          <KpiMini icon={Users}     label="Total utilizatori" value={users.length} />
          <KpiMini icon={Shield}    label="Admini"            value={users.filter(u => u.role_name === 'admin').length} />
          <KpiMini icon={UserCheck} label="Manageri"          value={users.filter(u => u.role_name === 'manager').length} />
          <KpiMini icon={KeyRound}  label="Roluri"            value={roles.length} />
        </div>
      </div>
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden enter-up" style={{ animationDelay: '160ms' }}>
          {}
          <div className="w-96 border-r border-line bg-surface-secondary flex flex-col min-h-0">
            <div className="px-4 py-3 border-b border-line">
              <h2 className="text-pm-2xs font-semibold uppercase tracking-wide text-content-muted">{users.length} Utilizatori</h2>
            </div>
            <div className="flex-1 min-h-0 overflow-y-auto">
              {users.map(u => (
                <button key={u.id} onClick={() => selectUser(u)}
                  className={`w-full text-left px-4 py-3 border-b border-line flex items-center gap-3 hover:bg-surface-tertiary/30 transition-colors ${selected?.id === u.id ? 'border-l-2 border-l-accent bg-accent/5' : ''}`}>
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-pm-2xs font-bold text-surface-primary ${ROLE_AVATAR_BG[u.role_name] || '[background:var(--role-hala-solid)]'}`}>
                  ...
                  <span className={`text-pm-2xs px-2 py-0.5 ${ROLE_COLORS[u.role_name] || 'bg-surface-tertiary text-content-secondary'}`}>
          ...
          <div className="flex-1 min-h-0 overflow-y-auto">
            {!selected ? (
              <div className="flex flex-col items-center justify-center h-full text-content-muted">
                ...
                <p className="text-sm">Selectează un utilizator</p>
              </div>
            ) : (
              <div className="flex flex-col">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-line bg-surface-secondary">
                  <div className="flex items-center gap-3">
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center text-pm-2xs font-bold text-surface-primary ${ROLE_AVATAR_BG[selected.role_name] || '[background:var(--role-hala-solid)]'}`}>
                      {getInitials(selected.full_name || selected.username)}
                    </div>
                    <div>
                      <h3 className="text-lg font-semibold text-content-primary">{selected.full_name}</h3>
                      {selected.job_title && (
                        <p className="text-pm-xs text-accent font-medium">{selected.job_title}</p>
                      )}
                      <p className="text-sm text-content-muted">@{selected.username} — {selected.email}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <button onClick={() => openModal(selected)} className="h-8 bg-surface-tertiary px-3.5 text-xs font-semibold text-content-primary hover:bg-surface-tertiary/80 flex items-center gap-1 transition-colors">
                      <Pencil className="h-3.5 w-3.5" /> Editează
                    </button>
                    <button onClick={() => handleDelete(selected.id)} className="p-1.5 text-content-muted hover:bg-status-red/10 hover:text-status-red transition-colors">
                ...
                {}
                <div className="border-b border-line bg-surface-secondary px-6 py-4">
                  <div className="flex items-center gap-2 mb-1">
                    <span className={`text-xs px-2 py-0.5 ${ROLE_COLORS[selected.role_name] || 'bg-surface-tertiary text-content-secondary'}`}>
                      {ROLE_LABELS[selected.role_name] || selected.role_name}
                    </span>
                    <StatusBadge tone={selected.active ? 'success' : 'neutral'} label={selected.active ? 'Activ' : 'Inactiv'} />

                  </div>
                  <p className="text-xs text-content-muted mt-1">Rolul determina ce pagini vede implicit. Bifeaza mai jos pentru a adăuga/scoate pagini individuale.</p>
                </div>
                ...
                <div className="bg-surface-primary px-6 py-4 border-t border-line">
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    <button
                      type="button"
                      onClick={() => setAllDashboardWidgets(true)}
                      className="text-pm-2xs px-2 py-0.5 border border-line text-content-muted hover:bg-surface-tertiary"
                    >Tot vizibil</button>
                    <button
                      type="button"
                      onClick={() => setAllDashboardWidgets(false)}
                      className="text-pm-2xs px-2 py-0.5 border border-line text-content-muted hover:bg-surface-tertiary"
                    >Tot ascuns</button>
                    <span className="ml-auto text-pm-2xs text-content-muted">
                      {Object.values(dashboardWidgets).filter(Boolean).length} / {DASHBOARD_WIDGETS.length} vizibile
                    </span>
                  </div>
                    ...
                    {DASHBOARD_WIDGETS.map(w => {
                      ...
                      return (
                        <li key={w.id} className="border-b border-line/30 last:border-b-0">
                          <label className="flex items-center gap-2 px-2 py-1.5 cursor-pointer hover:bg-surface-tertiary/30">
                            <input
                              type="checkbox"
                              checked={visible}
