...
import { FolderKanban, Factory, Warehouse, Loader2, Sparkles, TrendingUp, TrendingDown, Minus, Bell, Activity, RefreshCw, Wallet, Receipt } from 'lucide-react';
...
export default function DashboardPage({ user, onNavigate }: DashboardPageProps) {
  ...
  useEffect(() => {
    const stopDash = startDashPoll(5000);
    const stopHand = startHandoffPoll(5000);
  ...
  return (
    <Page>
      <div className="dash-shell flex-1 min-h-0 flex flex-col">
        <DashboardBackground />
        <main className="dashboard relative z-[1] flex-1 min-h-0 overflow-y-auto" style={{ scrollbarGutter: 'stable' }}>
          <div className="dash-canvas">

            {}
            <section className="dash-hero dash-enter dash-airy" style={{ animationDelay: '0ms' }}>
              <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-5">
                <div className="flex items-center gap-4 min-w-0">
                  <span className="h-14 w-14 rounded-2xl bg-accent/10 flex items-center justify-center shrink-0">
                    <Activity className="h-6 w-6 text-accent" />
                  </span>
                  <div className="min-w-0">
                    <h1 className="text-pm-3xl font-semibold text-content-primary truncate">Bună, {userFirst}</h1>
                ...
                <button onClick={handleRefresh} disabled={refreshing}
                  className="inline-flex items-center gap-1.5 h-10 px-4 rounded-lg border border-line/70 bg-surface-tertiary/40 text-pm-xs font-medium text-content-secondary hover:bg-surface-tertiary active:scale-[0.98] transition-all disabled:opacity-50 self-start shrink-0">
            ...
            {widgets.kpi_strip && (
              <div className="dash-enter" style={{ animationDelay: '100ms' }}>
                <div className="dash-kpi-feature">
                  <KpiFeature
                    label="Profit net" icon={Wallet} amount={profit} format={money} warn={profit < 0}
                    trend={margin >= 0 ? 'up' : 'down'}
                    trendLabel={`${margin >= 0 ? '+' : ''}${margin.toFixed(1)}%`}
                    noAccess={!canFinance}
                  />
                  <div className="dash-kpi-feature__splits">
                    <KpiSplit icon={TrendingUp} label="Venituri" amount={d.revenue} format={money} noAccess={!canFinance} />
                    <KpiSplit icon={FolderKanban} label="Proiecte active" amount={d.active_projects} format={fmtCount} unit={`/ ${d.total_projects}`} noAccess={!canProjects} />
                    <KpiSplit icon={Receipt} label="Costuri" amount={d.costs} format={money} noAccess={!canFinance} />
                  </div>
                </div>
                {canFinance && (
                  <p className="mt-3 text-center text-pm-2xs text-content-muted">Valori financiare în {displayCurrency} · curs {formatNumber(eurRate, 4)} RON/EUR</p>
                )}
              </div>
            )}

            {}
            <section className="dash-chart-hero dash-enter dash-airy" style={{ animationDelay: '320ms' }}>
              {widgets.revenue_chart && canFinance
                ...
                : <ZoneEmpty icon={TrendingUp} text="Fără date financiare de afișat" />}
            </section>

            {}
            <div className="dash-row-secondary">
              <section className="dash-enter dash-airy dash-airy--hover" style={{ animationDelay: '380ms' }}>
                <div className="dash-zone__head">
                  <FolderKanban className="h-3.5 w-3.5 text-content-muted" /><span className="dash-zone__title">Proiecte active</span>
                  {canProjects && <button onClick={() => onNavigate('projects')} className="ml-auto text-pm-xs text-accent hover:underline">Deschide</button>}
                </div>
                {!canProjects ? <ZoneEmpty icon={FolderKanban} text="Fără acces" />
                  : activeProjects.length === 0 ? <ZoneEmpty icon={FolderKanban} text="Niciun proiect activ" />
                  : (
              ...
              <section className="dash-enter dash-airy dash-airy--hover" style={{ animationDelay: '440ms' }}>
                <div className="dash-zone__head">
                  <Bell className="h-3.5 w-3.5 text-content-muted" /><span className="dash-zone__title">Alerte</span>
                  {d.pending_alerts > 0 && <span className="text-pm-2xs font-bold tabular-nums text-status-amber">{d.pending_alerts}</span>}
                  {can('alerts') && <button onClick={() => onNavigate('alerts')} className="ml-auto text-pm-xs text-accent hover:underline">Toate</button>}
                </div>
                {!can('alerts') ? <ZoneEmpty icon={Bell} text="Fără acces" />
                  : recentAlerts.length === 0 ? <ZoneEmpty icon={Bell} text="Nicio alertă activă" />
                  : (
                    <ul className="space-y-1">
                      {recentAlerts.map(a => (
                        <li key={a.id}>
            ...
            {}
            <div className="dash-row-footer">
              <section className="dash-enter dash-airy" style={{ animationDelay: '500ms' }}>
                <div className="dash-zone__head"><Factory className="h-3.5 w-3.5 text-content-muted" /><span className="dash-zone__title">Producție pe etape</span></div>
                {!canProduction ? <ZoneEmpty icon={Factory} text="Fără acces" />
                  : projectsByStage.length === 0 ? <ZoneEmpty icon={Factory} text="Fără date" />
                  : (
              ...
              <section className="dash-enter dash-airy" style={{ animationDelay: '540ms' }}>
                <div className="dash-zone__head"><Activity className="h-3.5 w-3.5 text-content-muted" /><span className="dash-zone__title">Operațional</span></div>
            ...
            {hasSecondary && (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {widgets.ai_summary && aiConnected && (
                  <div className="dash-airy">
                    <div className="dash-zone__head"><Sparkles className="h-3.5 w-3.5 text-accent" /><span className="dash-zone__title">Sinteză AI</span></div>
                    {aiSummaryLoading
                ...
                {widgets.briefing && <div className="dash-airy overflow-hidden"><DailyBriefingWidget onNavigate={onNavigate} /></div>}
                {widgets.inbox && (
                  <div className="dash-airy overflow-hidden">
                ...
                {widgets.critical_stock && canWarehouse && (
                  <div className="dash-airy">
                    <div className="dash-zone__head">
                      <Warehouse className="h-3.5 w-3.5 text-content-muted" /><span className="dash-zone__title">Stoc critic</span>
                      <button onClick={() => onNavigate('materials')} className="ml-auto text-pm-xs text-accent hover:underline">Deschide</button>
                    </div>
                    {criticalStock.length === 0 ? <ZoneEmpty icon={Warehouse} text="Niciun material sub minim" />
                      : (
  ...
  return (
    <div className={embedded ? '' : 'bg-surface-primary border-b border-line'}>
      <div className={`flex flex-wrap items-center gap-2 ${embedded ? '' : 'px-4 py-2.5'}`}>
        <span className="text-pm-2xs font-semibold uppercase tracking-wide text-content-muted">Interval</span>
...
function KpiFeature({ label, icon: Icon, amount, format, warn, trend, trendLabel, noAccess }: {
  label: string; icon: IconCmp;
  amount?: number; format?: (n: number) => string;
  warn?: boolean; trend?: 'up' | 'down' | 'flat'; trendLabel?: string; noAccess?: boolean;
}) {
  const TrendIcon = trend === 'up' ? TrendingUp : trend === 'down' ? TrendingDown : Minus;
  const animated = useCountUp(amount ?? 0);
  const shown = noAccess ? 'NaN' : (format ? format(animated) : fmtCount(animated));
  return (
    <div className="dash-kpi-feat dash-airy dash-airy--hover">
      <Icon className="dash-kpi-feat__wm" />
      <div className="relative z-[1] flex items-start justify-between gap-3">
        <p className="text-pm-xs font-bold uppercase tracking-[0.16em] text-content-muted">{label}</p>
        <span className="h-12 w-12 rounded-2xl bg-accent/12 text-accent flex items-center justify-center shrink-0">
          <Icon className="h-5 w-5" />
        </span>
      </div>
      <div className="relative z-[1]">
        <p className={`text-[40px] sm:text-[52px] font-semibold tabular-nums leading-[1.04] ${noAccess ? 'text-content-muted' : warn ? 'text-status-red' : 'text-content-primary'}`}
          style={{ fontFeatureSettings: "'tnum'" }}>
          {shown}
        </p>
        {noAccess ? (
          <p className="mt-2 text-pm-xs text-content-muted">nu ai acces la acest tip de date</p>
        ) : trend && trendLabel ? (
          <span className={`dash-kpi__trend-bounce mt-4 inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-pm-sm font-semibold ${
            trend === 'up' ? 'bg-status-green/12 text-status-green' : 'bg-status-red/12 text-status-red'
          }`}>
            <TrendIcon className="h-4 w-4" /> {trendLabel}
            <span className="font-normal text-content-muted">vs. perioada anterioară</span>
          </span>
        ) : null}
      </div>
    </div>
...
function KpiSplit({ icon: Icon, label, amount, format, unit, warn, noAccess }: {
  icon: IconCmp; label: string;
  amount?: number; format?: (n: number) => string;
  unit?: string; warn?: boolean; noAccess?: boolean;
}) {
  const animated = useCountUp(amount ?? 0);
  const shown = noAccess ? 'NaN' : (format ? format(animated) : fmtCount(animated));
  return (
    <div className="dash-kpi-split dash-airy dash-airy--hover">
      <span className="h-12 w-12 rounded-2xl bg-accent/10 text-accent flex items-center justify-center shrink-0">
        <Icon className="h-5 w-5" />
      </span>
      <div className="min-w-0">
        <p className="text-pm-2xs font-bold uppercase tracking-[0.12em] text-content-muted truncate">{label}</p>
        <p className="mt-1.5 flex items-baseline gap-1.5">
          <span className={`text-[26px] font-semibold tabular-nums leading-none ${noAccess ? 'text-content-muted' : warn ? 'text-status-red' : 'text-content-primary'}`}
            style={{ fontFeatureSettings: "'tnum'" }}>
            {shown}
          </span>
          {!noAccess && unit && <span className="text-pm-2xs text-content-muted">{unit}</span>}
        </p>
      </div>
    </div>
  );
...
function ZoneEmpty({ icon: Icon, text }: { icon: IconCmp; text: string }) {
  return (
    <div className="flex flex-1 flex-col items-center justify-center py-8 text-content-muted">
      <Icon className="h-6 w-6 opacity-30 mb-2" />
      <p className="text-pm-sm">{text}</p>
    </div>
  );
}
...
function MiniStat({ label, value, warn, accent }: {
  ...
  return (
    <div className="rounded-md bg-surface-tertiary/40 p-3">
