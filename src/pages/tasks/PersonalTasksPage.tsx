...
export default function PersonalTasksPage({ user }: { user: User | null }) {
  ...
  const openInfo = async (t: Task) => {
    ...
    try {
      const fresh = await apiCommand<Task>('get_personal_task', { id: t.id });
      setInfoTask(fresh);
    } catch {
      setInfoTask(t);
    }
    ...
    <Page className="mod-shell">
      <div className="mod-canvas">
        ...
        <HeroHeader
          ...
          subtitle="TODO list personal, task-uri delegate și mențiuni primite"
          actions={<Button size="sm" onClick={openCreate}><Plus className="h-3.5 w-3.5" /> Task nou</Button>}
        />

        {}
        <div className="mod-kpis enter-up" style={{ animationDelay: '80ms' }}>
          <KpiMini icon={ListTodo} label="Deschise"         value={openCount} />
          <KpiMini icon={Calendar} label="Scadente azi"     value={dueTodayCount} warn={dueTodayCount > 0} />
          <KpiMini icon={Clock}    label="Săptămâna asta"   value={dueWeekCount} />
          <KpiMini icon={UserPlus} label="Delegate de mine" value={delegatedOpen} />
        </div>
          ...
          <GlassCard size="regular" className="enter-up !p-0 overflow-hidden" style={{ animationDelay: '160ms' }}>
            <div className="flex items-center justify-between gap-3 flex-wrap px-5 pt-5 pb-3">
              <AnimatedTabs
                ...
                tabs={[
                  { id: 'tasks', label: `Ale mele (${openCount})` },
                  { id: 'delegated', label: `Delegate (${delegatedOpen})` },
                  { id: 'statusuri', label: `Statusuri${reviewBuckets.total > 0 ? ` (${reviewBuckets.total})` : ''}`, icon: Inbox },
                ]}
              />
              {(mainTab === 'tasks' || mainTab === 'delegated') && (
                <label className="text-pm-xs flex items-center gap-1.5 text-content-muted cursor-pointer">
            ...
            <div className="density-compact px-5 pb-5">
              {loading ? (
                ...
                tasks.length === 0 ? (
                  <EmptyState icon={CheckSquare} title="Niciun task activ" description="Adaugă un task nou ca să începi." />
                ) : (
                  <div className="space-y-2">
                    {(showDone ? tasks : tasks.filter(t => t.status !== 'done')).map(t => (
                      <TaskCard key={t.id} task={t} canEdit
                ...
                delegated.filter(t => showDone || t.status !== 'done').length === 0 ? (
                  <EmptyState icon={UserPlus} title="Nu ai delegat task-uri" description="Creează un task și asignează-l cuiva." />
                ) : (
                  <div className="space-y-2">
                    {delegated.filter(t => showDone || t.status !== 'done').map(t => (
                      <TaskCard key={t.id} task={t} canEdit showAssignee
          ...
          <div className="mod-aside enter-up" style={{ animationDelay: '240ms' }}>
            <GlassCard size="regular" className="!p-0 overflow-hidden">
              <div className="flex items-center justify-between gap-2 px-5 pt-5 pb-3">
                ...
                {unread > 0 && <button onClick={markAllRead} className="text-pm-xs text-accent hover:underline shrink-0">Toate citite</button>}
              </div>
              <div className="density-compact px-5 pb-5">
                {loading ? (
                  ...
                  <div className="space-y-2">
                    {mentions.map(m => {
                      ...
                      return (
                        <div key={m.id} className={`glass-surface rounded-lg p-3 ${m.is_read ? 'opacity-60' : 'border-accent/30'}`}>
...
function TaskCard({
  task, canEdit = false, showAssignee = false,
  onToggle, onInfo, onEdit, onDelete,
}: {
  task: Task; canEdit?: boolean; showAssignee?: boolean;
  ...
  const completion = task.completion_status
    ...
    : null;
  return (
    <div className={`glass-surface rounded-lg hover-lift p-3 flex items-start gap-3 ${
      task.status === 'done'
      ...
      <div className="flex items-center gap-0.5 shrink-0">
        {onInfo && (
          <button onClick={onInfo} title="Detalii" className="p-1 text-content-muted hover:bg-surface-tertiary hover:text-accent">
            <Info className="h-3.5 w-3.5" />
          </button>
        )}
        {canEdit && onEdit && (
          <button onClick={onEdit} title="Editează" className="p-1 text-content-muted hover:bg-surface-tertiary hover:text-accent">
            <Edit2 className="h-3.5 w-3.5" />
          </button>
        )}
        {onDelete && (
          <button onClick={onDelete} title="Șterge" className="p-1 text-content-muted hover:bg-surface-tertiary hover:text-status-red">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
      <div className="bg-surface-secondary rounded-lg border border-line shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-5 py-3 border-t border-line flex justify-end gap-2 bg-surface-primary/40">
          <button onClick={onClose} className="h-8 px-3 border border-line text-xs font-semibold text-content-secondary hover:bg-surface-tertiary">
            Anulează
          </button>
          <button onClick={onSave} className="h-8 px-4 bg-accent text-xs font-semibold text-surface-primary hover:opacity-90">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
      <div className="bg-surface-secondary rounded-lg border border-line shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-5 space-y-4">
          ...
          {isDone && canEdit && (
            <div className="border-t border-line pt-4">
              <button onClick={onReopen} className="h-8 px-3 border border-line text-xs font-semibold text-content-secondary hover:bg-surface-tertiary">
                Redeschide task-ul
              </button>
            </div>
          )}
      ...
      {delegatorSections.map(s => s.items.length > 0 && (
        <div key={s.key} className={`bg-surface-primary border-l-2 ${s.tone} pl-3`}>
          ...
          <div className="space-y-1.5">
            {s.items.map(t => (
              <div key={t.id} className="glass-surface rounded-lg p-3 flex items-start gap-3">
                <div className="flex-1 min-w-0">
        ...
        <div className="bg-surface-primary border-l-2 border-l-status-blue pl-3">
          ...
          <div className="space-y-1.5">
            {buckets.waiting_response.map(t => (
              <div key={t.id} className="glass-surface rounded-lg p-3">
                <p className="text-sm font-medium text-content-primary">{t.title}</p>
                <div className="flex items-center gap-2 mt-1 flex-wrap">
                  {t.assigned_by_name && (
      ...
      <div className="bg-surface-secondary rounded-lg border border-line shadow-xl w-full max-w-xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-5 py-3 border-t border-line flex flex-wrap items-center justify-end gap-2 bg-surface-primary/40">
          ...
          {status === 'unresolved' ? (
            <>
              {reassignTo ? (
                <button onClick={onReassign} className="h-8 px-3 bg-accent text-xs font-semibold text-surface-primary hover:opacity-90 inline-flex items-center gap-1.5">
                  <UserPlus className="h-3.5 w-3.5" /> Reasignează
                </button>
              ) : (
                <button onClick={onConfirm} className="h-8 px-3 bg-status-red/80 text-xs font-semibold text-white hover:opacity-90 inline-flex items-center gap-1.5">
            ...
            <>
              <button onClick={onSendBack} className="h-8 px-3 bg-status-amber/80 text-xs font-semibold text-white hover:opacity-90 inline-flex items-center gap-1.5">
                <Reply className="h-3.5 w-3.5" /> Trimite înapoi
              </button>
              <button onClick={onConfirm} className="h-8 px-3 bg-status-green text-xs font-semibold text-white hover:opacity-90 inline-flex items-center gap-1.5">
      ...
      <div className="bg-surface-secondary rounded-lg border border-line shadow-xl w-full max-w-lg" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-5 py-3 border-t border-line flex justify-end gap-2 bg-surface-primary/40">
          ...
          <button
            onClick={onSubmit}
            disabled={!text.trim()}
            className="h-8 px-3 bg-status-blue text-xs font-semibold text-white hover:opacity-90 disabled:opacity-40 inline-flex items-center gap-1.5"
          >
