    ...
    <Modal isOpen={isOpen} onClose={handleClose} title={title} size="md">
      <form onSubmit={handleSubmit} className="space-y-4" noValidate>
        {fields.map((field, idx) => {
          ...
          const inputClass = `w-full px-3 py-2 bg-surface-primary border ${
            showError ? 'border-status-red' : 'border-line hover:border-line/80'
          } rounded-md text-sm text-content-primary placeholder:text-content-muted/70 transition-colors duration-150 focus:outline-none focus:ring-2 ${
            showError ? 'focus:ring-status-red/40 focus:border-status-red' : 'focus:ring-accent/40 focus:border-accent'
          }`;
          ...
          return (
            <div key={field.name}>
              {showSection && (
                <h3 className="text-pm-eyebrow text-content-muted mt-2 mb-2 pt-2 border-t border-line first:border-t-0 first:pt-0 first:mt-0">
                  {field.section}
                </h3>
              )}
              ...
              {showError && (
                <p id={`${field.name}-error`} className="mt-1 flex items-center gap-1 text-pm-xs text-status-red">
                  <AlertCircle className="h-3 w-3 shrink-0" aria-hidden /> {fieldError}
                </p>
              )}
              {!showError && field.hint && (
                <p id={`${field.name}-hint`} className="mt-1 text-pm-2xs text-content-muted">{field.hint}</p>
              )}
            </div>
          );
        })}
        ...
        {formError && (
          <div className="flex items-start gap-2 p-3 bg-status-red/10 border border-status-red/30 rounded" role="alert">
            <AlertCircle className="mt-0.5 h-4 w-4 shrink-0 text-status-red" aria-hidden />
            <p className="text-xs text-status-red font-medium leading-relaxed">{formError}</p>
          </div>
        )}
        ...
        <div className="flex gap-3 pt-4">
          <button
            ...
            onClick={handleClose}
            disabled={loading}
            className="flex-1 px-4 py-2 bg-surface-tertiary text-content-primary rounded text-sm font-medium hover:bg-line transition-colors disabled:opacity-50"
          >
            Anulează
          </button>
          <button
            type="submit"
            disabled={loading}
            className="flex-1 px-4 py-2 bg-accent text-[var(--color-on-accent)] rounded-md text-sm font-semibold hover:shadow-[var(--glow-accent)] hover:brightness-105 transition-all disabled:opacity-50 disabled:shadow-none flex items-center justify-center gap-2"
          >
